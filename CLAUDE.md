...
- Every quote page must include Schema.org Quotation structured data (JSON-LD) with: @type Quotation, text, creator (Person with name and birthDate), dateCreated, isPartOf (CreativeWork source), and inLanguage. This enables Google featured snippets for 'who said' queries and voice assistant integration. _(from: Quotle Info Pages — Core Platform)_
  - **`description` was dropped from the creator requirement on 2026-07-29, deliberately.** `DOSSIER_SCHEMA` (`workflows/generate.js`) has no `creatorDescription` field, so no wave can emit one — the 48 records that carry a description got it by hand, against 1,030 with a creator at all. Adding the field costs 39 bytes and the schema sits at **exactly** its proven-good ceiling of 4,072 (`SCHEMA_BUDGET`, `tools/verify-corpus.js`), with zero headroom and nothing droppable that isn't doing constraint work. So the requirement was unmeetable by construction, and a repo that asserts a rule it cannot keep is worse off than one that states the narrower rule truthfully. `creator.jobTitle` is what actually ships (1,010 of 1,030) and `template.js` still passes a hand-written `description` through where a record has one — this narrows the RULE, it does not remove a capability.
