...
SCOPE — YOU MAY EDIT EXACTLY ONE FILE: ${DIR}/${p.slug}.json. Nothing else. Not tools/*, not workflows/*, not another record, not built HTML.
  ...
  NAMED INERT EDIT — TO CHANGE JSON-LD \`Quotation.text\`, SET \`schema.quotationText\`, NOT \`presentation.verifiedQuote\`. The r34 audit told an agent to fix the JSON-LD quote text via verifiedQuote. That field is read in exactly one place, kitQuote() at tools/template.js:1060, which builds the copy/share kit; Quotation.text comes from \`schema.quotationText || fullQuote || displayQuote\` at tools/template.js:209. So writing verifiedQuote alone LOOKS like a fix, passes review, and ships byte-identical markup. The two fields are both legitimate and they do different jobs — verifiedQuote lengthens what the kit copies while the H1 and slug stay clipped. If an issue names the wrong one, fix the right one and say so in your summary.
