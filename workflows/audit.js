...
- DOSSIER_SCHEMA cannot emit \`misattribution.items[].kind\`, so waves must hand-patch it. Declined 2026-08-07: the field is DERIVED, not asked of an agent — tools/mis-kind.js decides it, workflows/prep-wave.js stamps it at ingest, and of 482 rows the rule says are owed one, 0 are missing it. Adding it to the schema costs +22 to +61 bytes against ZERO headroom and would replace a skeptic-checked rule with per-wave agent judgement. See the "ASKED FOR AND DECLINED" comment in workflows/generate.js. A row that renders the WRONG marker is still a finding — file that as a mis-kind.js pattern gap, naming the row.
