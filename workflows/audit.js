...
6. CONTRACT: canonical, og:url, and the JSON-LD @ids all derive from ${ORIGIN}/who-said/${p.slug}. Report any mismatch. Note (do not fail on) the orchestrator template showing "quote undefined" — that is not the page.
   SETTLED, do NOT flag: ClaimReview.itemReviewed (a Claim node) carries no \`author\`, deliberately. In schema.org and Google's fact-check guidance \`Claim.author\` is whoever MADE the claim under review — the propagation vector — NOT the person the quote is falsely pinned on. Naming the credited person there asserts that they authored the claim about themselves, which on a genuine-attribution page is simply false; it is the same bug that put a false machine-readable claim on 59 pages. The credited name belongs in \`claimReviewed\`, where it already is. This has been re-filed by successive audits and refused by fix agents every time (correctly) — see the note at tools/template.js above the \`claimReview\` literal.
