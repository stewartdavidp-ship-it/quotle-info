...
const inner = `    <nav class="breadcrumb" aria-label="Breadcrumb"><a href="/">Home</a><span class="sep">›</span>Check a quote</nav>
    <main id="main">
        ...
        <p class="how">How this works: your text is matched (in your browser) against the ${'${COUNT}'} quotes we&rsquo;ve researched &mdash; traced to a primary source, or marked where none could be found. A match returns our verdict, the correct credit, and reuse status; no match means we haven&rsquo;t verified that exact line &mdash; which isn&rsquo;t proof it&rsquo;s fake, only that you shouldn&rsquo;t present it as confirmed. Agents can call the same data at <a href="/verify-index.json">/verify-index.json</a>, the <a href="/verify?q=your+quote">/verify API</a>, or POST a whole deck to <code>/verify-batch</code>.</p>
        <p class="how how-legal">Reuse status is our research, not legal advice &mdash; and copyright varies by country and edition. This page is the fastest way to catch a misattribution before you publish; for commercial or published use, confirm rights with the rightsholder. See <a href="/terms/">terms</a>.</p>
    </main>
            ...
            var rows=res.map(function(r,i){
                ...
                var link=(r.found&&r.url)?' <a href="'+esc(r.url)+'">details</a>':'';
                  ...
                  if(r.found&&r.rights){
                      ...
                      var rlab=rt==='public-domain'?'Free to use':rt==='in-copyright'?'In copyright':'Rights unconfirmed';
