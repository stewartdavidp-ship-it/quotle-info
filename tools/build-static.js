...
${STATES.map(stateRow).join('\n')}
        ...
        <section aria-labelledby="rights-h">
            ...
            <div class="states">
                ...
                <div class="state">
                    ...
                    <p class="state-def">We could not establish the status &mdash; usually because the wording is a translation or edition whose date or renewal we could not pin down. Treat it as <strong>not cleared</strong> until you check. It is the most common answer across the whole corpus, and stating it is the point: a confident wrong answer is worse than an honest gap.</p>
                </div>
            </div>
        </section>
        ...
        <section aria-labelledby="wrong-h">
            ...
            <p class="big"><a class="cta" href="${standingPath('report')}">Flag a correction or a missing quote &rarr;</a></p>
...
const privacyBody = `        <section aria-labelledby="p-sum">
        ...
        <section aria-labelledby="p-collect">
            ...
            <h3 class="sub">Community features (voting, reporting &amp; nominating)</h3>
            <p>If you vote a quote up, report a correction, or nominate one for review, those are optional actions. To keep them free of bots and spam we use <a href="https://www.cloudflare.com/products/turnstile/" rel="nofollow">Cloudflare Turnstile</a> and we store a <strong>salted, one-way hash of your IP address</strong> to enforce rate limits &mdash; we never store your raw IP, and the hash can&rsquo;t be reversed back to it. Anything you type into a report or nomination (the quote, a name, a link, a note) is stored so a human can review it before anything is published.</p>
            <h3 class="sub">Your email, if you give us one</h3>
            <p>The report and nomination forms have an <strong>optional</strong> email field. It is used for one thing: <strong>replying to you once, if we act on what you sent</strong> &mdash; that is, if we verify a correction and change a record, or add a quote you sent to our research list. If we look into it and nothing changes, we don&rsquo;t write; we&rsquo;d rather send you nothing than send you noise. It is stored alongside the report, never sold, never shared, never added to a mailing list, and never used to contact you about anything else. Leave it blank and the report still counts.</p>
        </section>
