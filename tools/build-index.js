...
const nomForm = `
            <form class="nom" id="nomForm" aria-label="Nominate a quote or author">
                ...
                <textarea class="nom-in nom-area" name="quote" maxlength="600" rows="2" placeholder="The quote, if you have it (optional)"></textarea>
                <textarea class="nom-in nom-area" name="note" maxlength="600" rows="3" placeholder="Anything you know about the real source (optional)"></textarea>
...
const reportBody = INTERACTIVE ? `        <header class="page-head">
        ...
        <section class="report-sec" id="wrong" aria-labelledby="wrong-h">
            ...
            <form class="nom" id="fixForm">
                ...
                <textarea class="nom-in nom-area" name="note" maxlength="600" rows="3" placeholder="Optional: anything else we should know"></textarea>
...
const REPORT_JS = INTERACTIVE ? `    <script>
        (function(){
            ...
            if(fx){ fx.addEventListener('submit',function(e){ e.preventDefault();
              ...
              function nominate(author,quote,note,msg,btn,f,okMsg){
                  token(function(t){ if(!t){ btn.disabled=false; show(msg,'Verification failed \\u2014 please try again.',true); return; }
                      ...
                      .then(function(r){return r.json();}).then(function(d){ if(d&&d.ok){ f.reset(); btn.disabled=false; show(msg,okMsg||'Thank you \\u2014 added to the review queue.',false); } else { btn.disabled=false; show(msg,(d&&d.error)||'Something went wrong.',true); } })
                      .catch(function(){ btn.disabled=false; show(msg,'Network error \\u2014 please try again.',true); }); });
              }
              if(f){ f.addEventListener('submit',function(e){ e.preventDefault();
                  ...
                  .then(function(r){return r.json();}).then(function(d){
                      ...
                      if(st==='nominated'){ nominate(author,quote,note,msg,btn,f,'Someone beat you to it \\u2014 already nominated. We have added your notes to it.'); return; }
                      ...
                      if(st==='wikiquote'&&d.added){ nominate(author,quote,note,msg,btn,f,'Thank you \\u2014 confirmed against Wikiquote and added to the review queue.'); return; }
